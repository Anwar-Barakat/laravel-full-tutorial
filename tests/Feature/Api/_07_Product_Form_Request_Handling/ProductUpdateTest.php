...
class ProductUpdateTest extends BaseProductApiTest
{
    ...
    public function test_product_update_does_not_require_all_fields()
    {
        $this->createAuthenticatedUser();
        $product = $this->createProduct();
        ...
        $response->assertStatus(200)
            ->assertJson([
                ...
                'data' => [
                    'name' => 'Only Name Updated',
                    'description' => $product->description, // Should remain unchanged
                    'price' => $product->price,             // Should remain unchanged
                ]
            ]);
    }
